...
def api_products():
    ...
    try:
        ...
        for rec in raw["product"]:
            ...
            margin_raw = f.get("利润率")
            margin = _num(margin_raw, default=0.4) if margin_raw is not None and margin_raw != "" else 0.4
...
def api_update_plan_info(record_id):
    ...
    try:
        ...
        # 预计交期
        if "override_delivery_date" in data:
            val = (data["override_delivery_date"] or "").strip()
            update_fields["预计交期"] = val
            cache_updates["预计交期"] = val
...
def api_client_plan_detail(record_id):
    ...
    try:
        ...
        # 交期：自动取最长产品交期（解析"XX天"格式）
        import re as _re
        max_days = 0
        for _p in products:
            _lt = _p.get("lead_time", "")
            _m = _re.search(r"(\d+)", str(_lt))
            if _m:
                _d = int(_m.group(1))
                if _d > max_days:
                    max_days = _d
        ...
        auto_delivery = ""
        if max_days > 0:
            auto_date = _dt.date.today() + _dt.timedelta(days=max_days)
            auto_delivery = auto_date.strftime("%Y-%m-%d")

        override_delivery = f.get("预计交期", "") or ""
        ...
        return jsonify({
            ...
            "products": products,
            "auto_delivery_date": auto_delivery,
            "override_delivery_date": override_delivery,
        })
    except Exception as e:
        traceback.print_exc()
        return jsonify({"ok": False, "error": str(e)})
